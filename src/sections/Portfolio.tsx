      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <article
              key={index}
              className="bg-white rounded-bento-lg shadow-bento overflow-hidden group hover:scale-[1.02] hover:shadow-bento-hover transition-all duration-300"
            >
              {/* Project Image/Icon Area */}
              <div className={`h-48 bg-gradient-to-br ${getGradient(project.image)} flex items-center justify-center text-bento-text relative overflow-hidden`}>
                <div className="absolute inset-0 bg-black/20" />
                <div className="relative z-10 opacity-80 group-hover:opacity-100 transition-opacity">
                  {getIcon(project.image)}
                </div>
                {/* Category Badge */}
                <span className="absolute top-4 left-4 px-3 py-1 bg-black/40 backdrop-blur-sm text-bento-text text-xs rounded-full">
                  {project.category}
                </span>
              </div>
              ...
              {/* Project Content */}
              <div className="p-6">
                <h3 className="text-xl font-bold text-bento-text mb-2">{project.title}</h3>
                <p className="text-bento-text-secondary mb-4">{project.description}</p>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tech.map((tech, tIndex) => (
                    <span
                      key={tIndex}
                      className="px-3 py-1 bg-bento-card text-bento-text-secondary text-xs rounded-full"
                    >
                      {tech}
                    </span>
                  ))}
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-4">
          <a
            ...
            className="inline-flex items-center gap-3 px-6 py-3 bg-bento-card text-bento-text-secondary rounded-full hover:bg-bento-card-hover transition-all"
          >
