const Courses = () => {
  const borderColors = ['#C67B5C', '#6B7B3C', '#B5651D', '#D4C4A8', '#C67B5C', '#6B7B3C'];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {courses.map((course, index) => (
            <article
              key={index}
              className={`rounded-bento-lg p-8 relative overflow-hidden transition-all duration-500 ${
            ...
            >
              ...
              {course.popular && (
                <div className="absolute top-0 right-0">
                  <div
                    className="text-xs font-bold px-4 py-1 rounded-bl-bento"
                    style={{ backgroundColor: '#C67B5C', color: '#FEFDFB' }}
                  >
                    熱門推薦
                  </div>
                </div>
              ...
              {course.official && (
                <div className="absolute top-0 right-0">
                  <div
                    className="text-xs font-medium px-3 py-1 rounded-bl-bento"
                    style={{ backgroundColor: '#6B7B3C', color: '#FEFDFB' }}
                  >
                    實際開課紀錄
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="mb-6">
                    <span
                      ...
                      style={{ color: course.official ? '#6B7B3C' : '#C67B5C' }}
                    >
                      {course.subtitle}
                    </span>
                    <h3
                  ...
                  {/* Description */}
                  <p className="mb-6" style={{ color: '#6B6255' }}>{course.description}</p>
                  ...
                  <ul className="space-y-3 mb-6">
                    {course.features.map((feature, fIndex) => (
                      <li key={fIndex} className="flex items-start gap-3">
                        ...
                        <span style={{ color: '#6B6255' }}>{feature}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className={course.official ? 'flex flex-col justify-between' : ''}>
                  ...
                  {course.official && course.officialMeta && (
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center gap-3 text-sm" style={{ color: '#6B6255' }}>
                        <svg className="w-4 h-4" style={{ color: '#6B7B3C' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex flex-wrap gap-4 mb-6 text-sm">
                    <div className="flex items-center gap-2" style={{ color: '#6B6255' }}>
                  ...
                  {course.hasMaterial ? (
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-full font-medium text-sm transition-all duration-400"
                      style={{ backgroundColor: '#C67B5C', color: '#FEFDFB' }}
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-2 font-medium transition-colors duration-400"
                      style={{ color: '#6B7B3C' }}
                    >
                      課程資訊頁面
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 font-medium transition-colors duration-400 cursor-pointer"
                      style={{ color: '#C67B5C' }}
                    >
                      聯繫諮詢
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="mt-16 text-center">
          <p className="mb-6" style={{ color: '#6B6255' }}>
            需要客製化課程或企業內訓？歡迎與我聯繫討論您的需求
          </p>
          <button
            onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
            className="btn-primary inline-flex items-center gap-2 px-8 py-4 rounded-full font-medium"
          >
            <span>預約免費諮詢</span>
