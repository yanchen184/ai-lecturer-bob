const Skills = () => {
  ...
  return (
    <section id="skills" className="py-20 lg:py-32 relative" aria-labelledby="skills-title" style={{ background: 'linear-gradient(180deg, rgba(15,21,33,0.5) 0%, rgba(10,14,23,1) 100%)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="font-mono text-neon-green/50 text-sm mb-2">{'$ analyze --skills --verbose'}</div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {skillCategories.map((category, catIndex) => (
            <div key={catIndex} className="glass-card p-6 hover:border-neon-green/25 transition-all duration-300 group">
              ...
              <div className="space-y-4">
                {category.skills.map((skill, skillIndex) => (
                  <div key={skillIndex}>
                    <div className="flex justify-between mb-1"><span className="text-gray-400 text-sm font-mono">{skill.name}</span><span className="text-neon-green text-sm font-mono">{skill.level}%</span></div>
        ...
        <div className="mt-12 glass-card p-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[{ name: 'React / TypeScript', level: 88 }, { name: 'Tailwind CSS', level: 85 }, { name: 'JavaScript / ES6+', level: 90 }].map((skill, index) => (
              <div key={index}>
                <div className="flex justify-between mb-1"><span className="text-gray-400 text-sm font-mono">{skill.name}</span><span className="text-neon-purple text-sm font-mono">{skill.level}%</span></div>
                <div className="skill-bar"><div className="skill-bar-fill" style={{ width: `${skill.level}%` }} /></div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-16 text-center">
          <h3 className="text-white font-semibold mb-6 font-mono text-neon-green/80">// 其他專業領域</h3>
          <div className="flex flex-wrap justify-center gap-3">
            {['NLP / NER 分析', 'Hugging Face', 'API 設計', '事件驅動架構', '高併發系統設計', '技術文件撰寫', '簡報技巧', '課程設計', '企業顧問', 'ChatGPT 輔助開發', 'Git Flow', 'Agile / Scrum'].map((tag, index) => (
              <span key={index} className="px-4 py-2 text-gray-400 rounded-full text-sm font-mono hover:text-neon-green transition-all duration-300 cursor-default" style={{ background: 'rgba(0,255,136,0.03)', border: '1px solid rgba(0,255,136,0.1)' }}>{tag}</span>
            ))}
          </div>
        </div>
      </div>
