    ...
    <>
          ...
          <div className="flex items-center justify-between h-16 md:h-18">
            <Link to="/" className="flex items-center space-x-2 group flex-shrink-0" aria-label="回到首頁">
              ...
              <div className="hidden sm:block">
                <span className="text-base font-bold" style={{ color: '#6366F1' }}>AI講師</span>
            ...
            <div className="hidden lg:flex items-center space-x-1">
              {homeNavItems.map((item) => (
                <a
                  ...
                  className={`relative px-3 py-1.5 text-sm rounded-lg transition-all duration-200 ${isActive(item.href) ? 'text-white font-bold' : 'hover:text-white'}`}
              ...
              <Link to="/blog" className={`px-3 py-1.5 text-sm rounded-lg transition-all duration-200 ${location.pathname.startsWith('/blog') ? 'text-white font-bold' : 'hover:text-white'}`} style={location.pathname.startsWith('/blog') ? { borderBottom: '2px solid #6366F1' } : { color: '#9CA3AF' }}>部落格</Link>
              <Link to="/#contact" className="ml-4 px-5 py-2 text-sm rounded-lg text-white font-bold transition-all duration-300" style={{ background: '#6366F1', boxShadow: '0 4px 15px rgba(99,102,241,0.25)' }}
                onMouseEnter={(e) => { e.currentTarget.style.background = '#818CF8'; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = '#6366F1'; }}
              >預約諮詢</Link>
            </div>

            <button className="lg:hidden relative w-10 h-10 flex items-center justify-center transition-colors duration-200" style={{ color: '#9CA3AF' }} onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label="切換選單" aria-expanded={isMobileMenuOpen}
      ...
      <div className={`fixed inset-0 z-40 lg:hidden transition-all duration-200 ${isMobileMenuOpen ? 'visible' : 'invisible'}`}>
        ...
        <div
          ...
          style={{ background: 'rgba(10,10,10,0.95)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderLeft: '1px solid #1F1F1F' }}
        >
          <div className="flex flex-col h-full pt-20 pb-8 px-6">
            <div className="flex-1 space-y-0">
              {homeNavItems.map((item, index) => (
                <a key={item.href} href={isHomePage ? item.href : `/#${item.href.replace('#', '')}`} onClick={(e) => handleHomeNavClick(e, item.href)} className="flex items-center px-4 py-3 text-base transition-all duration-200" style={{ borderBottom: '1px solid #1F1F1F', transitionDelay: isMobileMenuOpen ? `${index * 30}ms` : '0ms', opacity: isMobileMenuOpen ? 1 : 0, transform: isMobileMenuOpen ? 'translateX(0)' : 'translateX(20px)', color: isActive(item.href) ? '#FFFFFF' : '#9CA3AF', fontWeight: isActive(item.href) ? 700 : 400 }}>
                  {isActive(item.href) && <span className="w-2 h-0.5 mr-3 rounded-full" style={{ background: '#6366F1' }} />}
                  {item.label}
                </a>
              ))}
              <Link to="/blog" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center px-4 py-3 text-base transition-all duration-200" style={{ borderBottom: '1px solid #1F1F1F', color: location.pathname.startsWith('/blog') ? '#FFFFFF' : '#9CA3AF', fontWeight: location.pathname.startsWith('/blog') ? 700 : 400 }}>部落格</Link>
            </div>
            <div className="pt-4" style={{ borderTop: '1px solid #1F1F1F' }}>
              <Link to="/#contact" onClick={() => setIsMobileMenuOpen(false)} className="block w-full py-3 rounded-xl text-white font-bold text-center transition-all duration-300" style={{ background: '#6366F1' }}>預約諮詢</Link>
            </div>
          </div>
        </div>
      </div>
