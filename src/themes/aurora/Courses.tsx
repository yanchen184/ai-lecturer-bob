  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
        ...
        >
          <span className="chapter-label">03 — 課程</span>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {courses.map((course, index) => (
            <article
              ...
              className={`scroll-reveal glass-card p-8 relative overflow-hidden transition-all duration-600 ${
                course.popular ? 'ring-1 shadow-lg' : ''
              } ${course.official ? 'md:col-span-2' : ''}`}
              style={{
                transitionDelay: `${(index % 2) * 100}ms`,
                ...(course.popular ? { ringColor: 'rgba(233,69,96,0.4)', boxShadow: '0 0 20px rgba(233,69,96,0.1)' } : {}),
              }}
            >
              {/* Badge */}
              {course.popular && (
                <div className="absolute top-0 right-0">
                  <div className="text-white text-xs font-bold px-4 py-1 rounded-bl-lg" style={{ background: '#e94560' }}>
                    熱門推薦
                  </div>
                </div>
              )}
              ...
              <div className={course.official ? 'grid md:grid-cols-2 gap-8' : ''}>
                <div>
                  {/* Course Header */}
                  <div className="mb-6">
                    <span className={`text-sm font-medium ${course.official ? 'text-emerald-400' : ''}`} style={course.official ? {} : { color: '#e94560' }}>
                      {course.subtitle}
                    </span>
                  ...
                  {/* Description */}
                  <p className="mb-6" style={{ color: '#8892b0' }}>{course.description}</p>
                  ...
                  <ul className="space-y-3 mb-6">
                    {course.features.map((feature, fIndex) => (
                      <li key={fIndex} className="flex items-start gap-3">
                        <svg
                          className={`w-5 h-5 flex-shrink-0 mt-0.5 ${course.official ? 'text-emerald-400' : ''}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                          style={course.official ? {} : { color: '#e94560' }}
                        >
                ...
                <div className={course.official ? 'flex flex-col justify-between' : ''}>
                  ...
                  {course.official && course.officialMeta && (
                    <div className="space-y-3 mb-6">
                      <div className="flex items-center gap-3 text-sm" style={{ color: '#8892b0' }}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: '#0f3460' }}>
                  ...
                  <div className="flex flex-wrap gap-4 mb-6 text-sm">
                    <div className="flex items-center gap-2" style={{ color: '#8892b0' }}>
                  ...
                  {course.hasMaterial ? (
                    <a
                      ...
                      style={{ background: '#e94560', boxShadow: '0 0 15px rgba(233,69,96,0.2)' }}
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
