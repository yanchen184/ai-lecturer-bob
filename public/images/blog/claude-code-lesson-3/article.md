...
**Claude Code Skill / Subagent / Slash Command 三個東西長得像、其實完全不同**——Skill 是觸發式的小教學文件、Subagent 是有獨立 context 的小弟、Slash Command 是你定義的快捷指令。先把這三個拆乾淨,再上 `/loop`(連續迭代到滿意)、Hook(自動觸發 shell)、Remote Control(QR code 手機操控)、ralph-wiggum(辛普森家庭那個 Ralph,負責 max-iterate=20 的死磕迴圈)。Pro 訂閱者跑這些自動化任務剛好夠額度,不會吃光白天互動配額。

如果你搜「Claude Code Skill 怎麼寫」「Subagent vs Slash Command 差別」「Claude Code /loop 怎麼用」「ralph-wiggum 是什麼」找到這篇,你來對地方了。

## 目錄

- [第二堂的 CLAUDE.md 回家作業:有寫才有第三堂可以聽](#第二堂的-claudemd-回家作業有寫才有第三堂可以聽)
- [Skill vs Subagent vs Slash Command 怎麼選——三個容易混的東西](#skill-vs-subagent-vs-slash-command-怎麼選三個容易混的東西)
- [Claude Code Skill 怎麼寫——現場 demo 一個「週報 Skill」](#claude-code-skill-怎麼寫現場-demo-一個週報-skill)
- [Claude Code Model 切換:Sonnet 4.6 跟 Opus 4.7 什麼時候用](#claude-code-model-切換sonnet-46-跟-opus-47-什麼時候用)
- [/compact 跟 /clear 跟 Subagent 互動關係對照表](#compact-跟-clear-跟-subagent-互動關係對照表)
- [Claude Code Subagent 怎麼用——掃描桌面 202 個檔案 demo](#claude-code-subagent-怎麼用掃描桌面-202-個檔案-demo)
- [Claude Code /loop 怎麼用?每天 5:03 自動寄 AI 日報](#claude-code-loop-怎麼用每天-503-自動寄-ai-日報)
- [Claude Code Hook 怎麼設?任務跑超過 5 分鐘自動通知](#claude-code-hook-怎麼設任務跑超過-5-分鐘自動通知)
- [Claude Code Remote Control 怎麼用?手機掃 QR code 操控你電腦](#claude-code-remote-control-怎麼用手機掃-qr-code-操控你電腦)
- [ralph-wiggum 是什麼?max-iterate=20 的死磕迴圈工具](#ralph-wiggum-是什麼max-iterate20-的死磕迴圈工具)
- [常見問題 FAQ](#常見問題-faq)
- [延伸資源](#延伸資源)

## 第二堂的 CLAUDE.md 回家作業:有寫才有第三堂可以聽
...
## Skill vs Subagent vs Slash Command 怎麼選——三個容易混的東西
...
## Claude Code Skill 怎麼寫——現場 demo 一個「週報 Skill」
...
## Claude Code Model 切換:Sonnet 4.6 跟 Opus 4.7 什麼時候用
...
## /compact 跟 /clear 跟 Subagent 互動關係對照表
...
## Claude Code Subagent 怎麼用——掃描桌面 202 個檔案 demo
...
## Claude Code /loop 怎麼用?每天 5:03 自動寄 AI 日報
...
## Claude Code Hook 怎麼設?任務跑超過 5 分鐘自動通知
...
## Claude Code Remote Control 怎麼用?手機掃 QR code 操控你電腦
...
## ralph-wiggum 是什麼?max-iterate=20 的死磕迴圈工具
...
> 📅 **明天(2026-05-23)會發**:[Claude Code 第四堂——Superpowers + MCP + GitHub + Headless](/ai-lecturer-bob/blog/claude-code-lesson-4/)
> 第三堂玩過內建的 Skill / Subagent / Loop / Hook;第四堂視角翻轉——「我們自己寫一定不是最優解,要學會用別人寫的」。obra/superpowers 安裝、Twinkle Hub MCP 接台灣政府開放資料、GitHub + gh CLI auth、headless 模式半夜跑 batch。我卡關 3 次、被學員救場 1 次,完整紀錄。
...
- [系列上一篇:Claude Code 第二堂——馴服 CLAUDE.md + 十大 prompt 定義](/ai-lecturer-bob/blog/claude-code-lesson-2/)
- [系列下一篇:Claude Code 第四堂——Superpowers + MCP + GitHub](/ai-lecturer-bob/blog/claude-code-lesson-4/)(明天 2026-05-23 上線)
- [完整系列總覽:Claude Code Pro 訂閱者初階班 4 堂路線](/ai-lecturer-bob/blog/claude-code-pro-class-hub/)
- [Claude Code Skill 跟 Subagent 官方文件](https://docs.anthropic.com/en/docs/claude-code/sub-agents) — Skill 跟 Subagent 的官方差別說明
- [Claude Code Remote Control 官方文件](https://code.claude.com/docs/en/remote-control) — 手機 QR code 操控詳細 setup
- [Claude Code /loop /schedule 官方文件](https://code.claude.com/docs/en/loop-and-schedule) — autonomous mode 細節
- [Claude Sonnet 4.6 官方介紹](https://www.anthropic.com/news/claude-sonnet-4-6) — Claude Code 預設 model
- [Claude Opus 4.7 官方介紹](https://www.anthropic.com/news/claude-opus-4-7) — 升級 model
- [obra/superpowers-marketplace GitHub repo](https://github.com/obra/superpowers-marketplace) — ralph-wiggum 跟 superpowers 主 plugin 來源
- 同站延伸:
  - [Ralph Loop 實戰踩坑紀錄](/ai-lecturer-bob/blog/ralph-loop-real-world/) — ralph-wiggum 進階用法
  - [Claude Code Astro base path 跟 TaskList 兩個教訓](/ai-lecturer-bob/blog/claude-code-two-lessons-astro-and-tasklist/) — 我自己跑 Subagent 寫文章的踩坑
